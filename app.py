...
# Fallback to standard enterprise tracking key if production environment injection is missing
app.config['SECRET_KEY'] = os.getenv('SECRET_KEY', 'explorer_pay_enterprise_secure_gateway_node_2026')
...
SMTP_PORT = 465
SMTP_EMAIL = "artificialintelligence3070@gmail.com"
SMTP_PASSWORD = os.getenv('SMTP_PASSWORD', 'ftoa gqzk vzas ivzs')

# -------------------------------------------------------------------------
# GLOBAL PRODUCTION DATA STORAGE LAYERS
# -------------------------------------------------------------------------
# NOTE: Vercel serverless containers scale up/down dynamically. 
# Data inside these variables will reset when the instance goes cold.
MERCHANTS_DB = {}
PAYMENTS_DB = {}
WITHDRAWALS_DB = []
LINKS_DB = []
...
# Pre-seeded account for system verification
...
def route_merchant_dashboard():
    ...
    if not merchant:
        merchant = {
            "email": "N/A", "upi_vpa": "N/A", "balance": 0.00,
            "total_earnings": 0.00, "total_withdrawals": 0.00,
            "api_key": "N/A", "secret_key": "N/A"
        }
...
# -------------------------------------------------------------------------
# EXPOSE APPS WSGI BINDING LAYER FOR VERCEL HANDLER MODULES
# -------------------------------------------------------------------------
app = app

if __name__ == '__main__':
    # Fallback developer runtime setup (Termux/Local Environment Node)
    app.run(debug=True, port=9646)
